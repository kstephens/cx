    ...
    function(table_id_) {
      ...
      function parse_filter_(col_map, filter_str) {
        ...
        pats.forEach(function (pat) {
          ...
          var rx_fn = pat.rx_fn = pc.safe_rx_fn(pat.rx_str);
      ...
      function parse_filter(col_map, filter_str) {
        ...
        try {
          ...
          return function (row_data) { return false; };
        }
      }
      ...
      function rows_by_fn(trs, match_row_fn) {
        ...
        for (var i = 0; i < trs.length; i++) {
          ...
          if ( match_row_fn ) {
            ...
            if ( debug && matched )
              console.log("row_data matched : %j", row_data);
          }
          if ( matched ) {
            n_rows_visible ++;
            tr.style.display = '';
      ...
      function initalize(table_id_) {
        ...
        dom_matched_row_count = table.find(".cx-matched-row-count").toArray()[0];
