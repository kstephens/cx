...
module CX
  RSpec.describe Meta do
    include CX::Test
    subject { Meta.new }
    describe "min_max_size" do
      it "works" do
        expect(subject.min_size) .to eq(nil)
        expect(subject.max_size) .to eq(nil)

        subject.min_max_size! 5
        expect(subject.min_size) .to eq(5)
        expect(subject.max_size) .to eq(5)

        subject.min_max_size! 10
        expect(subject.min_size) .to eq(5)
        expect(subject.max_size) .to eq(10)
        
        subject.min_max_size! 0
        expect(subject.min_size) .to eq(0)
        expect(subject.max_size) .to eq(10)
      end
    end
